class EmailReminder
  ...
  def process_free_trial_1_week_left
    process(Time.now + 1.week, :free_trial_1_week) do |user|
      pp "Sending 1 week reminder to user: #{user.email} (#{user.id})" unless
        Rails.env == 'test'
      #pp user
  ...
  def process_free_trial_4_days_left    
    process(Time.now + 4.days, :free_trial_4_days) do |user|      
      pp "Sending 4 days reminder to user: #{user.email} (#{user.id})" unless 
        Rails.env == 'test'
      #pp user
  ...
  def process_free_trial_expire_today    
    process(Time.now, :free_trial_expiration) do |user|
      pp "Sending expiration reminder to user: #{user.email} (#{user.id})" unless
        Rails.env == 'test'
      #pp user
